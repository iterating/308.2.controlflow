function plantGrowth(startingPlants, thisWeek) {
  ...
  if (
    typeof startingPlants !== "number" ||
    isNaN(startingPlants) ||
    startingPlants <= 0
  ) {
  ...
  if (
    ...
    isNaN(thisWeek) ||
    thisWeek < 0 ||
    !Number.isInteger(thisWeek)
  ) {
    console.error(
      "The current week for plant growth must be a non-negative integer."
    );
    return;
  }
  ...
  console.log(
    `Space needed for plants: ${plantsTotalSpaceNow.toFixed(1)} square meters`
  );
...
console.log(
  `The radius of the area needed is ${radius.toFixed(1)} square meters`
);
